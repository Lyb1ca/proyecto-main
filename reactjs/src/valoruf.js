...
function Prestamo(props) {
    ...
    useEffect(() => {
        axios.get("https://mindicador.cl/api/uf").then((res) => {
            const data = res.data;
        ...
        axios.get("https://mindicador.cl/api/utm").then((res) => {
            const data = res.data;
            if (data.serie && data.serie.length > 0) {
        ...
        axios.get("https://mindicador.cl/api/euro").then((res) => {
            const data = res.data;
            if (data.serie && data.serie.length > 0) {
    ...
    const createFile = () => {
        ...
        var array =
            "Beneficiario: Juan Péres\nID de solicitud: 123456\nValor de la cuota en Pesos: 31034646.44" +
            "\nValor de la cuota en Uf: " +
            convUF +
            "\nValor de la cuota en UTM: " +
            convUTM +
            "\nValor de la cuota en Dolar: " +
            convDOLAR +
            "\nValor de la cuota en Euro: " +
            convEURO;
        ...
        <>
            <Card style={cardStyle}>
                <Card.Body>
                    ...
                    {valorUF && (
                        <Card.Text>
                            <strong>Valor UF:</strong> ${valorUF.toFixed(2)}{" "}
                        </Card.Text>
                    )}
                    {valorEURO && (
                        <Card.Text>
                            <strong>Valor EURO:</strong> ${valorEURO.toFixed(2)}{" "}
                        </Card.Text>
                    )}
                    {valorDOLAR && (
                        <Card.Text>
                            <strong>Valor DOLAR:</strong> $
                            {valorDOLAR.toFixed(2)}{" "}
                        </Card.Text>
                    )}
                    {valorUTM && (
                        <Card.Text>
                            <strong>Valor UTM:</strong> ${valorUTM.toFixed(2)}{" "}
                        </Card.Text>
                    )}
                    {fechaUF && (
                        <Card.Text>
                            <strong>Fecha UF:</strong> {formatDate(fechaUF)}
                        </Card.Text>
                    )}
                    {fechaUTM && (
                        <Card.Text>
                            <strong>Fecha UTM:</strong> {formatDate(fechaUTM)}
                        </Card.Text>
                    )}
                    {fechaEURO && fechaDOLAR && (
                        <Card.Text>
                            <strong>Fecha EURO y DOLAR:</strong>{" "}
                            {formatDate(fechaEURO)} ; {formatDate(fechaDOLAR)}
                        </Card.Text>
                    )}
                </Card.Body>
            </Card>
            <Card style={cardStyle}>
                <Card.Body>
                    ...
                    <Card.Text>
                        <strong>ID Solicitud:</strong> {props.idSolicitud}
                    </Card.Text>
                    <Card.Text>
                        <strong>Beneficiario:</strong> {props.beneficiario}
                    </Card.Text>
                    <Card.Text>
                    ...
                    <Card.Text>
                        <strong>Plazo (meses):</strong> {props.plazo}
                    </Card.Text>
                    <Card.Text>
                        <strong>Tasa Mensual (%):</strong> {props.tasa}
                    </Card.Text>
                    {cuota && (
                        <Card.Text>
                            <strong>Cuota (en moneda local):</strong> ${cuota}
                        </Card.Text>
                    )}
                    <Button
                        style={buttonStyle}
